...
const USAGE_PROVIDER_IDS = new Set(
  usageProviders.map((provider) => provider.id),
);
...
function normalizeCredentialIdsForDeletion(
  credentialIds: readonly string[],
): string[] {
...
function normalizeUsageRequestMaxAgeMs(
...
function inferQuotaStateFromUsage(
  snapshot: UsageSnapshot | null,
): UsageQuotaState {
  if (!snapshot) {
  ...
  if (snapshot.copilotQuota) {
    hasSignal = true;
    const buckets = [
      snapshot.copilotQuota.chat,
      snapshot.copilotQuota.completions,
    ...
    if (
      ...
      remainingValues.every((remaining) => remaining <= 0)
    ) {
      exhausted = true;
...
function getUsageSnapshotResetAt(
  snapshot: UsageSnapshot | null,
): number | null {
  if (!snapshot) {
    return null;
  ...
  const rateLimitResetAt = toEpochMs(
    snapshot.estimatedResetAt ?? snapshot.rateLimitHeaders?.resetAt,
  );
...
function cloneProviderState(
  ...
  return {
    ...
    cascadeState: state.cascadeState
      ? (JSON.parse(
          JSON.stringify(state.cascadeState),
        ) as ProviderRotationState["cascadeState"])
      : undefined,
    healthState: state.healthState
      ? (JSON.parse(
          JSON.stringify(state.healthState),
    ...
    pools: state.pools
      ? (JSON.parse(
        ...
        ) as ProviderRotationState["pools"])
      : undefined,
    poolConfig: state.poolConfig ? { ...state.poolConfig } : undefined,
    poolState: state.poolState ? { ...state.poolState } : undefined,
    chains: state.chains
      ? (JSON.parse(
          JSON.stringify(state.chains),
        ) as ProviderRotationState["chains"])
      : undefined,
    activeChain: state.activeChain
      ? (JSON.parse(
          JSON.stringify(state.activeChain),
        ) as ProviderRotationState["activeChain"])
      : undefined,
    quotaStates: state.quotaStates
      ? (JSON.parse(
          JSON.stringify(state.quotaStates),
        ) as ProviderRotationState["quotaStates"])
      : undefined,
  };
}
...
function remapNumericRecord(
  ...
  for (const [credentialId, value] of Object.entries(record)) {
    ...
    const nextCredentialId = resolveMappedCredentialId(
      credentialId,
      credentialIdMap,
      validIds,
    );
  ...
  const remapped = pools
    .map((pool) => ({
      ...
      credentialIds: pool.credentialIds
        .map((credentialId) =>
          resolveMappedCredentialId(credentialId, credentialIdMap, validIds),
        )
        .filter(
          (credentialId): credentialId is string => credentialId !== undefined,
        ),
      config: pool.config ? { ...pool.config } : undefined,
    }))
    .filter(
      (pool) => pool.poolId.trim().length > 0 && pool.credentialIds.length > 0,
    );

  return remapped.length > 0 ? remapped : undefined;
}

function resolveProviderPoolConfig(
  state: ProviderRotationState,
): ProviderPoolConfig {
  return {
    enablePools:
      state.poolConfig?.enablePools ?? (state.pools?.length ?? 0) > 0,
    failoverStrategy:
      state.poolConfig?.failoverStrategy ??
...
function applyCredentialNormalization(
  ...
  state.lastUsedAt = remapNumericRecord(
    state.lastUsedAt,
    result.credentialIdMap,
    validIds,
  );
  state.usageCount = remapNumericRecord(
    state.usageCount,
    result.credentialIdMap,
    validIds,
  );
  ...
  state.friendlyNames = remapStringRecord(
    state.friendlyNames,
    result.credentialIdMap,
    validIds,
  );
  state.pools = remapPools(state.pools, result.credentialIdMap, validIds);
  state.quotaStates = remapQuotaStates(
    state.quotaStates,
    result.credentialIdMap,
    validIds,
  );
...
function normalizeProviderState(state: ProviderRotationState): void {
  ...
  if (state.cascadeState) {
    for (const providerId of Object.keys(state.cascadeState)) {
      ...
      if (providerCascadeState.active) {
        providerCascadeState.active.cascadePath =
          providerCascadeState.active.cascadePath.filter((attempt) =>
            validIds.has(attempt.credentialId),
          );
        providerCascadeState.active.attemptCount =
          providerCascadeState.active.cascadePath.length;
      ...
      providerCascadeState.history = providerCascadeState.history
        .map((entry) => ({
          ...entry,
          cascadePath: entry.cascadePath.filter((attempt) =>
            validIds.has(attempt.credentialId),
          ),
          attemptCount: entry.cascadePath.filter((attempt) =>
            validIds.has(attempt.credentialId),
          ).length,
        }))
        .filter((entry) => entry.cascadePath.length > 0);
      if (
        !providerCascadeState.active &&
        providerCascadeState.history.length === 0
      ) {
        delete state.cascadeState[providerId];
      }
    }
  ...
  if (state.pools) {
    state.pools = state.pools
      .map((pool) => ({
        ...pool,
        credentialIds: pool.credentialIds.filter((credentialId) =>
      ...
      .filter(
        (pool) =>
          pool.poolId.trim().length > 0 && pool.credentialIds.length > 0,
      )
      .sort((left, right) => {
  ...
  if (state.poolState) {
    const poolExists =
      state.pools?.some(
        (pool) => pool.poolId === state.poolState?.activePoolId,
      ) ?? false;
    if (!poolExists) {
    ...
    if (
      ...
      (!Number.isInteger(state.poolState.poolIndex) ||
        state.poolState.poolIndex < 0)
    ) {
      state.poolState.poolIndex = 0;
    }
    if (
      !state.poolState.activePoolId &&
      state.poolState.poolIndex === undefined
    ) {
      state.poolState = undefined;
    }
  }
  ...
  if (
    state.activeIndex < 0 ||
    state.activeIndex >= state.credentialIds.length
  ) {
    state.activeIndex = 0;
  }

  if (state.manualActiveCredentialId) {
    const manualIndex = state.credentialIds.indexOf(
      state.manualActiveCredentialId,
    );
    if (manualIndex >= 0) {
...
function getDisabledError(
  ...
  return {
    ...
    disabledAt:
      typeof entry.disabledAt === "number" ? entry.disabledAt : Date.now(),
  };
}

function isLegacyCodexOAuthRefreshFailureMessage(
...
function migrateLegacyCodexRefreshDisabledCredentials(
  ...
  let changed = false;
  for (const [credentialId, entry] of Object.entries(
    ...
    const disabledAt =
      ...
      Number.isFinite(entry.disabledAt) &&
      entry.disabledAt > 0
        ? entry.disabledAt
        : now;
...
function clearRecoveredCodexRefreshFailureState(
  ...
  if (
    ...
    Object.keys(state.weeklyQuotaAttempts).length === 0
  ) {
    state.weeklyQuotaAttempts = {};
  }
...
export class AccountManager {
  ...
  private readonly extensionConfig: MultiAuthPicordConfig;
  ...
  private readonly runtimeOptions: Readonly<
    Required<AccountManagerRuntimeOptions>
  >;
  ...
  constructor(
    ...
    extensionConfig: MultiAuthPicordConfig = DEFAULT_MULTI_AUTH_CONFIG,
    ...
    this.runtimeOptions = {
      startOAuthRefreshScheduler:
        runtimeOptions.startOAuthRefreshScheduler !== false,
    };
    this.cascadeStateManager = new CascadeStateManager(
      this.extensionConfig.cascade,
    );
    ...
    this.keyDistributor.setModelEligibilityResolver(
      (providerId, credentialIds, modelId) =>
        this.resolveCredentialModelEligibility(
          providerId,
          credentialIds,
          modelId,
        ),
    ...
    this.oauthRefreshScheduler = new OAuthRefreshScheduler(
      async (credentialId, providerId) =>
        this.refreshScheduledOAuthCredential(providerId, credentialId),
  ...
  private async getCredentialUsageSnapshotWithContext(
    ...
    const usageRequestCacheKey = getUsageRequestCacheKey(
      provider,
      credentialId,
      options,
    );
    const existingUsageRequest =
      context?.selectionCache.usageByRequest.get(usageRequestCacheKey);
    if (existingUsageRequest) {
      return existingUsageRequest;
    }

    const usageRequest = this.loadCredentialUsageSnapshot(
      provider,
      credentialId,
      options,
      context,
    ...
    context.selectionCache.usageByRequest.set(
      usageRequestCacheKey,
      usageRequest,
    );
    return usageRequest;
  }
  ...
  private async loadCredentialUsageSnapshot(
    ...
    const cachedUsage = this.usageService.readCachedUsage(
      provider,
      credentialId,
      options,
    );
    ...
    const credential = await this.resolveCredentialForUsage(
      credentialId,
      context,
    );
    if (!credential) {
  ...
  async recordCredentialFailure(
    ...
    if (this.cascadeStateManager.hasActiveCascade(provider)) {
      this.cascadeStateManager.recordCascadeAttempt(
        provider,
        credentialId,
        errorKind,
        errorMessage,
      ...
      this.cascadeStateManager.createCascade(
        provider,
        credentialId,
        errorKind,
        errorMessage,
  ...
  async setFriendlyName(
    ...
    return this.storage.withLock((state) => {
      ...
      if (!providerState.credentialIds.includes(credentialId)) {
        throw new Error(
          `Credential ${credentialId} is not available for provider ${provider}`,
        );
      }
  ...
  async loginProvider(
    ...
    const backupResult = await this.authWriter.setOAuthCredentialAsBackup(
      provider,
      credentials,
    );
  ...
  private async persistCredentialList(
    ...
    await this.storage.withLock((state) => {
      ...
      const addedIndex = providerState.credentialIds.indexOf(
        lastAddedCredentialId,
      );
    ...
    const credential =
      persistedCredential ??
      (await this.authWriter.getCredential(lastAddedCredentialId));
    if (credential?.type === "oauth") {
  ...
  async switchActiveCredential(
    ...
    await this.storage.withLock((state) => {
      ...
      providerState.manualActiveCredentialId =
        providerState.credentialIds[index];
   ...
   */
  async clearManualActiveCredential(
    provider: SupportedProviderId,
  ): Promise<void> {
    await this.storage.withLock((state) => {
      const providerState = getProviderState(state, provider);
  ...
  async deleteCredentials(
    ...
    const normalizedCredentialIds =
      normalizeCredentialIdsForDeletion(credentialIds);
  ...
  private async disableCredential(
    ...
    if (!errorMessage) {
      throw new Error(
        "Cannot disable credential without a non-empty error message.",
      );
    }
  ...
  private async clearRecoveredOAuthRefreshFailureState(
    ...
    await this.storage.withLock((state) => {
      ...
      const changed = clearRecoveredCodexRefreshFailureState(
        providerState,
        credentialId,
      );
      if (!changed) {
  ...
  private async tryRecoverConcurrentCodexRefresh(
    ...
    const hasRotatedRefreshToken =
      currentCredential.refresh !== credential.refresh;
    const hasRotatedAccessToken =
      currentCredential.access !== credential.access;
  ...
  private async logAndHandleOAuthRefreshFailure(
    ...
    const failure = this.createOAuthRefreshFailure(
      provider,
      credentialId,
      error,
    );
  ...
  async reenableCredential(
    ...
    const credentialsById = await this.authWriter.getCredentials([
      credentialId,
    ]);
    const credential = credentialsById.get(credentialId);
    if (credential?.type === "oauth") {
      const refreshed = await this.refreshCredentialToken(
        provider,
        credentialId,
        credential,
      );
  ...
  async refreshProviderCredentials(
    ...
    for (const credentialId of state.credentialIds) {
      ...
      try {
        const usage = await this.getCredentialUsageSnapshot(
          provider,
          credentialId,
          {
            forceRefresh: true,
  ...
  async resolveFailoverTarget(
    ...
    return this.storage.withLock(async (state) => {
      ...
      const metadata =
        await this.providerRegistry.resolveProviderRegistrationMetadata(
          next.providerId,
        );
      if (!metadata) {
        return {
      ...
      const resolvedModel = metadata.models.find(
        (candidate) => candidate.id === next.modelId,
      );
      if (!resolvedModel) {
        return {
          result: null,
      ...
      this.persistFailoverStateToProviders(
        state,
        chainDefinitions,
        exportedState,
      );
    ...
    for (const providerState of Object.values(state.providers)) {
      for (const chain of providerState.chains ?? []) {
        ...
        if (!deduped.has(chain.chainId)) {
          deduped.set(
            chain.chainId,
            JSON.parse(JSON.stringify(chain)) as FailoverChain,
          );
        }
      }
    }
  ...
  private getActiveFailoverState(
    ...
    for (const providerState of Object.values(state.providers)) {
      if (
        providerState.activeChain &&
        chainIds.has(providerState.activeChain.chainId)
      ) {
        return JSON.parse(
          JSON.stringify(providerState.activeChain),
        ) as FailoverChainState;
      }
    }
    return undefined;
  }
  ...
  private buildQuotaState(
    ...
    return quotaClassifier.createQuotaState(
      credentialId,
      errorMessage,
      classification,
    );
  ...
  async acquireCredential(
    ...
    if (state.credentialIds.length === 0) {
      throw new Error(
        `No credentials available for ${provider}. Open /multi-auth and add an account.`,
      );
    }
    ...
    const effectiveExcludedCredentialIds = new Set(
    ...
    for (const blockedCredentialId of this.cascadeStateManager.getBlockedCredentialIds(
      provider,
    )) {
    ...
    const selectionCache =
      options?.selectionCache ?? createCredentialSelectionCache();
    ...
    if (selectedIndex === undefined) {
      let now = Date.now();
      let available = buildAvailableSet(
        state,
        now,
      ...
      if (available.size === 0) {
        const recoveredState =
          await this.releaseOneCooldownLockForProviderWithoutUsage(
            provider,
            state,
            effectiveExcludedCredentialIds,
          );
    ...
    await this.storage.withLock((stored) => {
      ...
      const nextRoundRobinIndex =
        providerState.credentialIds.length > 0
          ...
          : selectedIndex;
      if (providerState.manualActiveCredentialId) {
        providerState.activeIndex = selectedIndex;
      } else if (effectiveRotationMode === "round-robin") {
        providerState.activeIndex = nextRoundRobinIndex;
      } else {
        // For usage-based and balancer modes, still advance the index so that
        // the round-robin fallback in getRoundRobinCandidateIndex starts from
        // the next credential instead of re-scanning the one just used.
        providerState.activeIndex = nextRoundRobinIndex;
      }
      providerState.usageCount[credentialId] =
        (providerState.usageCount[credentialId] ?? 0) + 1;
      providerState.lastUsedAt[credentialId] = Date.now();
  ...
  async markQuotaExceeded(
    ...
    const normalizedMessage =
      errorMessage?.trim() ||
      (isWeekly ? "Weekly quota exhausted" : "Quota exhausted");
    const classifiedQuota = quotaClassification
      ? ({
          classification: quotaClassification,
          cooldownMs:
            typeof recommendedCooldownMs === "number" &&
            Number.isFinite(recommendedCooldownMs)
              ? recommendedCooldownMs
              : quotaClassifier.classifyFromMessage(normalizedMessage)
                  .cooldownMs,
          recoveryAction:
            quotaClassifier.getRecoveryAction(quotaClassification),
    ...
    let cooldownMs =
      typeof classifiedQuota.cooldownMs === "number" &&
      Number.isFinite(classifiedQuota.cooldownMs)
        ? classifiedQuota.cooldownMs
        : QUOTA_COOLDOWN_MS;
    const quotaState = this.buildQuotaState(
      credentialId,
      normalizedMessage,
      classifiedQuota,
    );
    ...
    const shouldApplyBalancerCooldown = await this.storage.withLock((state) => {
      ...
      providerState.lastQuotaError[credentialId] = normalizedMessage.slice(
        0,
        500,
      );
      ...
      if (weeklyQuota) {
        providerState.weeklyQuotaAttempts =
          providerState.weeklyQuotaAttempts ?? {};
        const attempts =
          (providerState.weeklyQuotaAttempts[credentialId] ?? 0) + 1;
        providerState.weeklyQuotaAttempts[credentialId] = attempts;
        cooldownMs = Math.max(cooldownMs, getWeeklyQuotaCooldownMs(attempts));
      } else if (
        providerState.weeklyQuotaAttempts?.[credentialId] !== undefined
      ) {
  ...
  async markTransientProviderError(
    ...
    const message =
      errorMessage.trim().slice(0, 500) || "Transient provider error";
    await this.recordCredentialFailure(
      provider,
      credentialId,
      0,
    ...
    const shouldApplyBalancerCooldown = await this.storage.withLock((state) => {
      ...
      const now = Date.now();
      const attempts =
        (providerState.transientErrorCount[credentialId] ?? 0) + 1;
   ...
   */
  async autoActivatePreferredCredentials(
    options: AutoActivateOptions = {},
  ): Promise<void> {
    await this.ensureInitialized();
    ...
    for (const provider of providers) {
          ...
          if (state.activeIndex !== manualIndex) {
            await this.storage.withLock((stored) => {
              ...
              const nextManualIndex =
                providerState.credentialIds.indexOf(manualCredentialId);
      ...
      await this.storage.withLock((stored) => {
        ...
        if (
          selectedIndex < 0 ||
          selectedIndex >= providerState.credentialIds.length
        ) {
          return { result: undefined, next: stored };
        }
  ...
  async getProvidersWithCredentials(): Promise<SupportedProviderId[]> {
    ...
    const providersWithCredentials = new Set(
      await this.authWriter.listProviderIds(providers),
    );
    return providers.filter((provider) =>
      providersWithCredentials.has(provider),
    );
  }
  ...
  async getStatus(): Promise<ProviderStatus[]> {
    ...
    const statuses = await Promise.all(
      providers.map((provider) => this.getProviderStatus(provider)),
    );
  ...
  private async getDisabledCredentialIds(
    state: ProviderRotationState,
  ): Promise<Set<string>> {
  ...
  private async releaseOneCooldownLockForProviderWithoutUsage(
    ...
    const didUpdate = await this.storage.withLock((stored) => {
      const providerState = getProviderState(stored, provider);
      const currentExhaustedUntil =
        providerState.quotaExhaustedUntil[candidateCredentialId];
      if (
        typeof currentExhaustedUntil !== "number" ||
        currentExhaustedUntil <= Date.now()
      ) {
        return { result: false };
      }
  ...
  private async getUsageBasedCandidateIndex(
    ...
    const hasUsageSignals = ranked.some(
      (candidate) => candidate.hasUsageSnapshot,
    );
    if (!hasUsageSignals) {
  ...
  private async reconcileQuotaStateFromUsage(
    ...
    const didUpdateState = await this.storage.withLock((state) => {
      ...
      const nextUntil = Math.max(
        ...
        quotaState.exhaustedUntil ??
          classificationResult.window?.windowEndMs ??
          fallbackUntil,
      );
      const currentUntil = providerState.quotaExhaustedUntil[credentialId] ?? 0;
      providerState.quotaExhaustedUntil[credentialId] = Math.max(
      ...
      providerState.quotaStates = providerState.quotaStates ?? {};
      providerState.quotaStates[credentialId] = {
        ...this.buildQuotaState(
          credentialId,
          "Quota inferred from usage snapshot",
  ...
  private deduplicateCredentialEntries(
    ...
    for (const [index, entry] of credentialEntries.entries()) {
      ...
      const identityKey = buildCodexIdentityKey(
        entry.credentialId,
        entry.credential,
      );
    ...
    return credentialIds.filter((credentialId) =>
      retainedCredentialIds.has(credentialId),
    );
  }

  private async syncProviderState(
    provider: SupportedProviderId,
  ): Promise<ProviderRotationState> {
    await this.ensureInitialized();
    const credentialEntries =
      await this.authWriter.getProviderCredentialEntries(provider);
    const credentialIds = credentialEntries.map((entry) => entry.credentialId);
    const normalizedCredentialIds = this.deduplicateCredentialEntries(
      provider,
      credentialEntries,
    );
    ...
    if (
      haveEquivalentProviderState(currentProviderState, normalizedProviderState)
    ) {
  ...
  async ensureInitialized(): Promise<void> {
    ...
    const initializationPromise = (async () => {
      ...
      const normalizedProviders =
        await this.authWriter.normalizeProviderCredentials(providers);
      ...
      const credentialEntriesByProvider = new Map<
        SupportedProviderId,
        readonly AuthCredentialEntry[]
      >();
      for (const provider of providers) {
        const credentialEntries =
          await this.authWriter.getProviderCredentialEntries(provider);
        credentialEntriesByProvider.set(provider, credentialEntries);
        const credentialIds = credentialEntries.map(
      ...
      const persistedProviders = await this.storage.withLock((state) => {
        for (const provider of providers) {
          const providerState = getProviderState(state, provider);
          providerState.credentialIds = [
            ...(credentialIdsByProvider.get(provider) ?? []),
          ];
          if (provider === "openai-codex") {
      ...
      for (const [provider, providerState] of Object.entries(
        persistedProviders,
      )) {
        this.loadProviderTelemetry(provider, providerState);
        await this.syncProviderOAuthSchedules(
  ...
  private async persistProviderTelemetry(
    provider: SupportedProviderId,
  ): Promise<void> {
    await this.storage.withLock((state) => {
      const providerState = getProviderState(state, provider);
      ...
      providerState.healthState = this.healthScorer.exportState(
        providerState.credentialIds,
      );
      normalizeProviderState(providerState);
      return { result: undefined, next: state };
  ...
  private scheduleOAuthRefresh(
    ...
    const expiration = determineTokenExpiration(
      credential.access,
      credential.expires,
    );
  ...
  private async syncProviderOAuthSchedules(
    ...
    for (const credentialId of Object.keys(
  ...
  private async persistOAuthRefreshSchedule(
    provider: SupportedProviderId,
  ): Promise<void> {
  ...
  private async refreshScheduledOAuthCredential(
    ...
    try {
      ...
      this.scheduleOAuthRefresh(provider, credentialId, refreshed);
      await this.persistOAuthRefreshSchedule(provider);
      return determineTokenExpiration(refreshed.access, refreshed.expires)
        .expiresAt;
    } catch (error) {
